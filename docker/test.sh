...
"###################################\n"\
&& docker build --quiet --tag dbt-core \
  --target dbt-core \
  docker \
...
"####################################\n"\
&& docker build --quiet --tag dbt-core-1.0.0b1 \
  --target dbt-core \
...
"#######################################\n"\
&& docker build --quiet --tag dbt-postgres \
  --target dbt-postgres \
  docker \
...
"########################################\n"\
&& docker build --quiet --tag dbt-postgres-1.0.0b1 \
  --target dbt-postgres \
...
"#######################################\n"\
&& docker build --quiet --tag dbt-redshift \
  --target dbt-redshift \
  docker \
...
"########################################\n"\
&& docker build --quiet --tag dbt-redshift-1.0.0b1 \
  --target dbt-redshift \
...
"#######################################\n"\
&& docker build --quiet --tag dbt-bigquery \
  --target dbt-bigquery \
  docker \
...
"########################################\n"\
&& docker build --quiet --tag dbt-bigquery-1.0.0b1 \
  --target dbt-bigquery \
...
"########################################\n"\
&& docker build --quiet --tag dbt-snowflake \
...
"#########################################\n"\
&& docker build --quiet --tag dbt-snowflake-1.0.0b1 \
  --target dbt-snowflake\
...
"####################################\n"\
&& docker build --quiet --tag dbt-spark \
  --target dbt-spark \
  docker \
